...
//   await deleteAsync(tmpBarPath, { force: true })
...
//   expect(
//     diff(foo.getCache().set('will-this-fire.txt', Buffer.from('hello')), result)
//       .size,
//   ).toBe(0)
...
//   const writtenBar: Snapshot = await bar.write({
//     'some-file.css': 'aside {background: squirple;}',
//     [path.join('another', 'file.css')]: 'figure {color: glue}',
//   })

//   expect(writtenBar.size).toBe(2)
//   expect(String(writtenBar.get('some-file.css'))).toBe(
//     'aside {background: squirple;}',
//   )
//   expect(String(writtenBar.get(path.join('another', 'file.css')))).toBe(
//     'figure {color: glue}',
//   )
