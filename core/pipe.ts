 ...
 *
 * Also casts to snapshot before and after each step.
 *
 * Tip: one convenient way to use this is passing `() => files` as the first argument (where `files` is an in-scope snapshot), so the resulting transform can be called with no arguments.
 *
 * @public
 */
